...
import Prerequisites from "../../src/components/SharedMarkdown/_prequisites.mdx";
...
<ProvisionAKSCluster version="1.32.0" />
...
We can see all of the nodes in both the system and user node pools are at version **1.30.0** as well.

```text
NAME                                 STATUS   ROLES    AGE    VERSION
aks-systempool-14753261-vmss000000   Ready    <none>   123m   v1.30.0
aks-systempool-14753261-vmss000001   Ready    <none>   123m   v1.30.0
aks-systempool-14753261-vmss000002   Ready    <none>   123m   v1.30.0
aks-userpool-27827974-vmss000000     Ready    <none>   95m    v1.30.0
```

Run the following command to get the latest supported version in the region.

```bash
export KUBERNETES_VERSION=$(az aks get-versions -l ${LOCATION} \
...
Run the following command to upgrade the current cluster API server, and the Kubernetes version running on the nodes.
