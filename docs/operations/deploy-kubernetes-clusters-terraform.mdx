...
We will build on our existing `main.tf` file, so add a new line after the `azurerm_resource_group` resource block and add code similar to the sample that is included in the provisioning instructions found on the module documentation page.
...
Just for kicks, we can also deploy a sample app with the following command:
