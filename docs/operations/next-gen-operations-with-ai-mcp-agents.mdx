...
### Challenge 1: Is store admin closed for business?
...
- CLI Agent: _"Users can't access store-admin in the pets namespace. Help me figure out why."_
- kagent: _"Investigate why no services are reachable in the pets namespace."_
- GitHub Copilot with AKS MCP: _"Why can't I reach any services in the pets namespace?"_
...
**Scenario**: With the store-admin site up and running again, users report that the AI-generated product descriptions feature isn't working. The feature relies on the ai-service pod making inference calls to Azure AI Foundry. Investigate the issue and get the AI-generated product descriptions working again.

This challenge has multiple layers -- fixing one problem may reveal another underneath. The challenge is complete when the ai-service pod logs show successful authentication and inference calls to Azure AI Foundry.

Use an agent to diagnose why the AI feature isn't working and fix it. Try one of these prompts to get started:

- CLI Agent: _"The AI-generated product descriptions feature in store-admin isn't working. Help me investigate the ai-service pod in the pets namespace."_
- kagent: _"The ai-service pod in the pets namespace isn't working correctly. Investigate why."_
- GitHub Copilot with AKS MCP: _"Why isn't the ai-service working in the pets namespace? Check its logs and configuration."_
...
- The **ai-service** is deployed with workload identity but is missing two things:
  - The **ServiceAccount** has a placeholder client ID annotation (`00000000-0000-0000-0000-000000000000`) -- it needs to point to a real managed identity
  - The **federated identity credential** for the ai-service ServiceAccount hasn't been created in Azure
- To fix this, create a federated credential linking the managed identity to the `ai-service` ServiceAccount in the `pets` namespace, and update the ServiceAccount annotation with the correct client ID
...
### Challenge 3: Beyond fixing -- hardening the cluster
