...
The kagent UI has left and right panels that are collapsible. To move them aside and give more room for the agent's response, click the vertical lines on the inside edges of the panels toggle them open and closed.
...
| **Agent Instructions**            | Accept the default instructions provided, but insert additional instructions at the top of the instruction section to `Use AKS MCP tools wherever possible and delegate tasks to appropriate agents when necessary.` |
...
> TODO: REPLACE `pauldotyu/aks-labs/refs/heads/ai_assisted_ops` WITH `azure-samples/aks-labs/refs/heads/main` ONCE PR REVIEW IS COMPLETED
