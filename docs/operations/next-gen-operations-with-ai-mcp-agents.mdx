...
Deploy a sample application to the cluster to have some workloads and telemetry to work with in the agents later on.

```bash
kubectl create namespace pets
kubectl apply -n pets -k "https://github.com/Azure-Samples/aks-store-demo//kustomize/overlays/dev?ref=main"
kubectl apply -n pets -f https://raw.githubusercontent.com/Azure-Samples/aks-store-demo/refs/heads/main/ai-service.yaml
```
...
### Intentional sabotaging of the cluster

To make the workshop more engaging and to give you a chance to see the diagnostic capabilities of the agents in action, we have intentionally sabotaged the cluster by creating a network policy that blocks all traffic to the kagent-ui service in the kagent namespace. This will create a connectivity issue that you can troubleshoot using the agents later in the lab.

Run the following command to create the network policy to disable ingress in the pets namespace.
...
Select the settings icon, then click the **Start Server** to ensure it is running.
...
```bash
AI_API_KEY=$(az cognitiveservices account keys list --name $AI_NAME --resource-group $RG_NAME --query key1 -o tsv)
AI_API_BASE=$(az cognitiveservices account show --name $AI_NAME --resource-group $RG_NAME --query 'properties.endpoints."OpenAI Language Model Instance API"' -o tsv)
...
- provider: azure
  ...
  AZURE_API_BASE: $AI_API_BASE
  AZURE_API_KEY: $AI_API_KEY
...
The agent calls a tool such as `k8s_describe_resource` with arguments targeting the `deployment` resource type and the name `kagent-ui` in the `kagent` namespace and summarizes teh results.
...
Before you deploy the MCP server to AKS, you will need to create a user-assigned managed identity so that it can work with your Azure services.
...
TODO: In case the above doesn't work for permissions, run this.

```sh
kubectl create clusterrolebinding aks-mcp-cluster-admin --clusterrole=cluster-admin --serviceaccount=kagent:aks-mcp
```
...
- **Tools & Agents**: Select **+ Add Tools & Agents**, then in the dialog, filter for `aks` and select all tools under **Kagent/Aks-Mcp**.
- TODO**Skills**: Add the following skills to enhance the agent's capabilities: `ghcr.io/pauldotyu/kubernetes-skill:v1.0.0`, `ghcr.io/pauldotyu/azure-skill:v1.0.0`

Click the **Save Selection (13)** button to confirm the tools.

Confirm all the details are correct, then scroll to the bottom of the page and click the **Create Agent** button to deploy the agent.

:::info

You may notice the agent status is "Agent not Accepted". Wait a few minutes for the agent to be created, then refresh the page.
...
The agent should run `aks_advisor_recommendation` for the current cluster and return a summary list of Azure Advisor recommendations. You should see a report with _HighAvailability (High)_ and _OperationalExcellence (Low)_ items with descriptions and status.
...
3. Scroll to **Tools & Agents**, then click the **+ Add Tools & Agents**.

![Add tools and agents to aks-agent](https://placehold.co/600x400)

5. Add the following agents, then click the **Save Selection (15)**.
...
```prompt
I seem to be unable to unable to access the store-front application deployed in the pets namespace. Can you investigate this?
```
...
- status check failed: [ciliumendpoints.cilium.io is forbidden: User "system:serviceaccount:kagent:aks-mcp" cannot list resource "ciliumendpoints" in API group "cilium.io" at the cluster scope]
- Error from server (Forbidden): ciliumnetworkpolicies.cilium.io is forbidden: User "system:serviceaccount:kagent:aks-mcp" cannot list resource "ciliumnetworkpolicies" in API group "cilium.io" at the cluster scope
