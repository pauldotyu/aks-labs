...
Verify the pods are running before continuing.

```bash
kubectl get pods -n pets
```

You should see pods for `makeline-service`, `order-service`, `product-service`, `store-front`, and others in a **Running** state. It may take a minute for all pods to start.
...
Keep your terminal open as you will need it to run commands using the environment variables you exported throughout the workshop. If you accidentally close it, you'll need to re-export `RG_NAME`, `LOCATION`, `RAND`, `AKS_NAME`, and `AI_NAME` before continuing.
...
In the GitHub Copilot Chat panel, expand the **MCP SERVERS - INSTALLED** section.
...
The Agentic CLI for AKS is an AI-powered command-line tool that helps AKS users troubleshoot cluster issues efficiently. It analyzes telemetry signals (logs, metrics, events), correlates them across infrastructure and workloads, and provides actionable insights through natural language queries.
...
### Install the Agentic CLI for AKS
...
Before using the agent, you need to configure the backend language model. The Azure AI Foundry service and model you provisioned earlier will power the agent's natural language understanding.

Run the following commands to create the agent configuration file. These settings tell the agent where to find the AI service and how to authenticate.
...
The **TEMPERATURE** setting controls how creative or predictable the AI responses are. The **gpt-5** family of models are highly capable reasoning models that only support a temperature value of 1.
...
- The agent retrieves container specs from each Deployment in the `pets` namespace.
- The response summarizes CPU and memory requests and limits per container.

To exit the CLI Agent TUI, select **Ctrl+C** or type `exit`.
...
kagent provides a CLI to install agents into your clusters. However, to use Azure AI Foundry models, you need Helm charts for more control over the configuration.
...
The port-forward command blocks the terminal. Keep it running and use a separate terminal for subsequent commands.
...
In your VS Code terminal, set up the managed identity and deploy the AKS MCP server.
...
Earlier, you used the pre-defined **k8s-agent** to inspect basic cluster state. Now comes the exciting part -- creating a new agent and connecting it to the AKS MCP server. This integration gives AI agents access to Azure-specific tools and diagnostics, enabling them to perform AKS operations, monitor resources, and troubleshoot issues using natural language.
...
- **Tools & Agents**: Select **+ Add Tools & Agents**, then in the dialog, filter for `aks` and select all tools under **Kagent/Aks-Mcp**.
  Select the **Save Selection** button to confirm the tools.
...
4. Add the following agents, then select **Save Selection**.
...
5. Scroll to the bottom of the page and select **Update Agent** to save the changes.

![Update the agent in kagent](https://placehold.co/600x400)

:::info

The AKS agent automatically delegates tasks to specialized agents (Kubernetes operations to k8s-agent, network policies to cilium-policy-agent). When you ask a question, you may see it delegating to the k8s-agent. This hierarchical architecture creates a more capable system where each agent contributes domain expertise.
...
```bash
curl -fsSL https://raw.githubusercontent.com/pauldotyu/aks-labs/refs/heads/main/docs/operations/assets/setup-cluster-issues.sh | bash
```
...
With the cluster broken, continue to the troubleshooting challenges.
...
### Challenge 1: Pods are failing and the AI feature is missing

**Scenario**: Users report that they can't access the store-front application in the pets namespace. Some pods aren't starting at all, and others are crashing or restarting. Additionally, the store-admin UI is missing its "AI-generated product descriptions" button -- the feature that uses the ai-service to generate marketing copy for products.

**Your tasks**:

1. Using one of the agent tools, investigate why the store application isn't responding
2. Identify the root causes across infrastructure and application layers
3. Fix the issues so all pods are healthy
4. Investigate the ai-service pod -- it's deployed with workload identity annotations, but the authentication to Azure AI Foundry is failing. Determine what's missing and fix it so the AI button appears in store-admin
5. Verify the application is fully operational and the AI feature works

**Where to start**:

- Try asking the CLI Agent: _"Why are pods in the pets namespace not healthy? Show me all failing pods, their events, and logs."_
- Or use kagent: _"Check pod status, events, and logs in the pets namespace. Look especially at the ai-service pod for authentication errors."_
- Or ask GitHub Copilot with the AKS MCP server: _"Check for network policies, pod health, and workload identity configuration in the pets namespace"_

<details>
<summary>Hints (expand if you're stuck)</summary>

- A **network policy** is blocking ingress traffic to the `pets` namespace (infrastructure level)
- Multiple **application deployments** have configuration issues:
  - One service references a **missing ConfigMap** and won't start
  - One service has a **broken liveness probe** and crashes
- The **ai-service** is deployed with workload identity but is missing two things:
  - The **ServiceAccount** has a placeholder client ID annotation -- it needs to point to a real managed identity
  - The **federated identity credential** for the ai-service ServiceAccount hasn't been created in Azure
  - Pod logs will show authentication failures when trying to get a token for Azure AI Foundry
- To fix the ai-service, you need to create a federated credential linking the managed identity to the `ai-service` ServiceAccount in the `pets` namespace, and update the ServiceAccount annotation with the correct client ID
...
### Challenge 2: The ai-service still can't reach Azure AI Foundry

**Scenario**: You resolved the pod and authentication issues from Challenge 1. But when you restart the ai-service pod, it still can't connect to Azure AI Foundry. The logs show DNS resolution failures or connection timeouts when trying to reach the AI Foundry endpoint URL. Other pods seem to have similar issues resolving external hostnames.

**Your tasks**:

1. Using an agent, check the ai-service pod logs for connection or DNS errors
2. Verify that DNS resolution is failing cluster-wide, not just for the ai-service
3. Investigate what's blocking DNS lookups -- look beyond Kubernetes into Azure infrastructure
4. Fix the issue
5. Restart the ai-service pod and verify the AI button now appears in the store-admin UI

**Where to start**:

- Try the CLI Agent: _"The ai-service pod still can't reach its external endpoint after the Challenge 1 fixes. Check the pod logs and help me figure out why."_
- Or use kagent with the AKS agent: _"Pods in the cluster are having trouble connecting to external services. Investigate what's going on."_
- Or ask GitHub Copilot: _"The ai-service pod can't connect to Azure AI Foundry even though authentication is fixed. Help me investigate the connectivity issue."_

<details>
<summary>Hint (expand if you're stuck)</summary>

This issue is at the Azure infrastructure level, not Kubernetes. An NSG rule in the cluster's managed resource group is blocking outbound traffic to Azure DNS (168.63.129.16). CoreDNS relies on this address to resolve external hostnames like the Azure AI Foundry endpoint. Remove or update the NSG rule, then restart the ai-service pod to verify connectivity.

</details>

---

### Challenge 3: Harden the cluster with multi-agent best practices

**Scenario**: Challenges 1 and 2 are resolved -- pods are healthy, DNS works, and the AI button appears in the store-admin UI. But the cluster was just "fixed," not hardened. The `pets` namespace may lack proper network segmentation, resource requests and limits could be missing or undersized, and security best practices like pod security standards haven't been reviewed.
...
1. Ask the aks-agent to perform a comprehensive best practices review of the `pets` namespace covering:
   - **Networking** -- Are there network policies in place? Is traffic restricted to only what's needed?
   - **Security** -- Are workload identity configurations correct? Are there any Azure Advisor security recommendations? Are pod security standards enforced?
   - **Resiliency** -- Do all Deployments have appropriate resource requests and limits? Are replicas configured for availability? Are liveness and readiness probes defined?
2. Review the agent's findings and have it implement the highest-priority recommendations
3. Verify the application still works after applying the changes

**Where to start**:

- In kagent, ask the aks-agent: _"Review the pets namespace for best practices across networking, security, and resiliency. Check for missing network policies, resource limits, pod disruption budgets, and security recommendations from Azure Advisor. Then implement the top recommendations."_
...
After implementing changes, port-forward the store-front and verify the application loads and the AI button in store-admin still generates product descriptions.
...
After completing the lab, remove the intentional cluster issues and all Azure resources.

### Clean up cluster issues

To restore the cluster to normal state, run the cleanup script:

```bash
curl -fsSL https://raw.githubusercontent.com/pauldotyu/aks-labs/refs/heads/main/docs/operations/assets/cleanup-cluster-issues.sh | bash
```
...
- The broken application patches (restores original aks-store-demo deployments)
- The custom `ai-service` ServiceAccount with workload identity annotations
