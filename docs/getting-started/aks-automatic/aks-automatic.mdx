...
The `--no-wait` flag runs the deployment in the background. This allows you to continue while the resources are being deployed.
...
![Kubernetes namespace](./assets/deploy-app-cluster-logging.png)
...
Navigate back to the **Conversation** tab and select the **Merge pull request** button to merge the pull request, then select **Confirm merge**.

![GitHub merge pull request](./assets/github-pull-request-merged.png)
...
Select the **Application Monitoring** tab to configure AutoInstrumentation for the contoso-air application.
...
When the VPA manifest is generated, select the **Accept all** button to accept the changes, then select **Add** to create the VPA resource.
