...
This workshop will show you how easy it is to deploy applications to AKS Automatic. AKS Automatic is a new way to deploy and manage Kubernetes clusters on Azure. It is a fully managed Kubernetes service that simplifies the deployment, management, and operations of Kubernetes clusters. With AKS Automatic, you can deploy a Kubernetes cluster with just a few steps in the Azure portal, and it is designed to be simple and easy to use, so you can focus on building your applications!
...
Log in to the [Azure portal](https://portal.azure.com), type **Kubernetes services** in the search box at the top of the page, and select the **Kubernetes services** option from the search results.
...
![AKS Automatic cluster creation](./assets/deploy-app-cluster-new.png)
...
With AKS Automatic, Deployment Safeguards are automatically applied to your cluster which includes a safeguard that prevents the use of the `SYS_PTRACE` capability in your application deployments via Kubernetes Pod Security Standards. This safeguard will block any deployment that includes this capability from being deployed to the cluster. By removing this capability from the manifest, you are ensuring that your application can be successfully deployed to the cluster while also adhering to security best practices. See [Kubernetes Pod Security Standards](https://kubernetes.io/docs/concepts/security/pod-security-standards/) for more information on pod security best practices.

:::

Place your mouse over the like where `SYS_PTRACE` is added and select the plus icon to add a comment to the line.

![GitHub pull request deployment manifest edit](./assets/github-pull-request-edit-manifest.png)

In the comment box, click the **Add a suggestion** button, then select the contents in the suggestion block and delete the line to remove the capability from the manifest. The suggestion block should now contain an empty line. Select the **Comment** button to post the comment and suggestion to the pull request.

![GitHub pull request deployment manifest edit](./assets/github-pull-request-edit-manifest-remove-sysptrace.png)
...
:::tip
If the deploy job fails, it is likely that Node Autoprovisioning (NAP) is taking a bit longer than usual to provision a new node for the cluster. Try selecting the "Re-run" button at the top of the page to re-run the deploy workflow job.
...
In the left-hand menu, select **Services and ingresses** under the **Kubernetes resources** section. You should see a new service called `contoso-air` with an external IP address assigned to it. Select the IP address to view the deployed application.
...
![AKS configmap](./assets/contoso-air-config.png)
...
![AKS configmap](./assets/contoso-air-config-edit.png)
...
In the left-hand menu, select **Service Connector** under **Settings** then select the **+ Create** button.
...
![AKS service connector authentication](./assets/service-connector-auth.png)
...
![AKS service connector authentication](./assets/service-connector-advanced.png)
...
In the Azure portal, navigate to the **Workloads** section under **Kubernetes resources** in the left-hand menu. Filter by the **dev** namespace and select the **contoso-air** deployment to view the deployment details.
...
![Application Insights resource](./assets/app-insights.png)

Select the **Application map** under **Investigate** in the left-hand menu to view a high-level overview of the application components, their dependencies, and number of calls.
...
![Application Insights resource](./assets/app-insights-performance.png)
...
To access this feature, navigate back to your AKS cluster in the Azure portal. You should see the **Monitoring** section in the left-hand menu. Select it to view a high-level summary of your cluster's performance.
...
To aid in troubleshooting your application, you can easily access container logs emitted by the application. Select **Logs** from the **Monitoring** section in the AKS cluster's left-hand menu. This section allows you to access the logs gathered by the Azure Monitor agent operating on the cluster nodes.
...
In the AKS cluster's left-hand menu, select **Dashboards with Grafana** under the **Monitoring** section. You will see a list of all the pre-configured Grafana dashboard that are available for your cluster.

![Browse dashboards](./assets/monitor-grafana.png)

Log into the Grafana instance then in the Grafana home page, select the **Dashboards** link in the left-hand menu. Here you will see a list of pre-configured dashboards that you can use to visualize the metrics collected by the Prometheus workspace.
...
![Grafana explore with PromQL](./assets/grafana-explore.png)
...
![Grafana explore with PromQL](./assets/grafana-explore-datasource.png)
...
![Grafana explore with PromQL](./assets/grafana-promql.png)
...
Navigate to the **Custom resource** section under **Kubernetes resources** in the AKS cluster left-hand menu. Scroll down to the bottom of the page and select the **Load more** button to view all the available custom resources.
...
Select the text editor or press **Alt + I** to open the Copilot editor.
...
Navigate to **Application scaling** under **Settings** in the AKS cluster left-hand menu, then select the **+ Create** button.
