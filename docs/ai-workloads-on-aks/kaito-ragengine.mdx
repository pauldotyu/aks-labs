...
The [KAITO Preset Workspace for the **phi-4-mini-instruct** model](https://github.com/kaito-project/kaito/blob/main/examples/inference/kaito_workspace_phi_4_mini.yaml) uses the `Standard_NC24ads_A100_v4` VM SKU. This lab aims for cost-effectiveness over performance, so a `Standard_NC4as_T4_v3` is used instead. You must have sufficient quota available in your Azure subscription to deploy the workspace. If you don't have at least 4 vCPUs of Standard NCASv3_T4 GPU quota, you will need to [request additional quota](https://learn.microsoft.com/azure/quotas/quickstart-increase-quota-portal) from Azure support.
