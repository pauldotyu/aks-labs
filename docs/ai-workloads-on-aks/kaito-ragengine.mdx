...
[KAITO workspaces](https://github.com/kaito-project/kaito/blob/main/website/docs/inference.md) are Kubernetes custom resources that are used to manage open-source model inference deployments within your cluster. Run the following command to create a workspace for using the **phi-4-mini-instruct** model:
...
[KAITO RAGEngine](https://github.com/kaito-project/kaito/blob/main/website/docs/rag.md) is another Kubernetes custom resource that simplifies the deployment and management of Retrieval Augmented Generation (RAG) workloads. It allows you to easily deploy RAG applications and workflows that can retrieve information from external knowledge sources and use it to enhance the responses generated by large language models (LLMs).
...
With KAITO, the complexity of managing vector databases, embedding models, and inference services is abstracted away, letting you focus on building applications rather than infrastructure.
...
## What's next? Expanding Your RAG Applications
